...
public class Arena : MonoBehaviour
{
    ...
    //pokemon count
    public void removePokemon(Team t){
        fixList(t);
        if(pokemonsA.Count == 0 || pokemonsB.Count == 0){
            playing = false;
        }
    ...
    void fixList(Team t){
        if(t == Team.A){
            for(int i = 0; i < pokemonsA.Count; i++){
                if(!pokemonsA[i].active){
                    
                    pokemonsA.RemoveAt(i);
                }
            }
            
        }
        if(t == Team.B){
            for(int i = 0; i < pokemonsB.Count; i++){
                if(!pokemonsB[i].active){
                    
                    pokemonsB.RemoveAt(i);
